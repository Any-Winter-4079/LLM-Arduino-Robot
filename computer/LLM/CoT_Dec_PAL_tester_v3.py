"""
GSM8K v3 Evaluator
Advanced framework for testing language models on mathematical reasoning tasks with multiple methods.
Features:
- Multiple reasoning strategies: Chain of Thought, Program-Aided, Declarative, and Zero-Shot
- Early stopping when consensus reached
- Configurable iterations per method
- Advanced answer selection with frequency-based decision making
- Comprehensive results tracking and analysis
- Support for custom datasets to avoid contamination
- Method-specific iteration tracking and seed management
- Detailed logging of reasoning paths and final decisions
"""
...
def test_DATASET():
    """
    Test the LLM on the DATASET using multiple reasoning methods with configurable iterations.
    
    Features early stopping when consensus is reached and comprehensive result tracking.
    """
...
def run_method(method, question, context, cpp=False, iteration=0):
    ...
    Args:
        method: Reasoning method to use (0Shot, CoT, Decl, PAL, or Decider)
        question: Question to process
        context: Context containing exemplars for the method
        cpp: Whether to use C++ implementation
        iteration: Current iteration number (affects seed)
        
    Returns:
        Tuple of (method name, detailed prediction, concise prediction)
    """
    if "0Shot" in method:
